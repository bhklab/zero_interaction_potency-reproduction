...
## Shouldn't HS lie in [0, 4]???
    ...
    fitted(estimate_param) ## like pred(), get predicted response from cruve
...
twowayfitting <- function(cor_matrix, drug_pair) {
    ...
    drug1_model <- drm(
        ...
        data = drug1,
        fct = L.4(
            ## So they did not fix E_min and E_max in fitting single agent??
            fixed = c(NA, NA, NA, NA),
            names = c("SLOPE", "MIN", "MAX", "IC50")
        ),
        logDose = 10,
    ...
    drug2_model <- drm(
        ...
        data = drug2,
        fct = L.4(
            fixed = c(NA, NA, NA, NA), ## kinda like cheating?
            names = c("SLOPE", "MIN", "MAX", "IC50")
        ),
        logDose=10,
        na.action=na.omit
    ...
    for (i in 2:ncol(cor_matrix)){
            ...
            if(tmp$fitted_inhibition[nrow(cor_matrix)-1] < 0)
                tmp$fitted_inhibition[nrow(cor_matrix)-1] <- tmp_min
